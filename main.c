...
long int default_register_value[] = {
    0,
    0,
    0,
    0,
        ...
        else if (strcmp(tokens_comm[0], "cache_sim") == 0 && strcmp(tokens_comm[1], "enable") == 0)
        {
            ...
            if (cache.associativity != 0)
            {
                ...
                for (int i = 0; i < cache.numsets; i++)
                {
                    ...
                    cache.sets[i].fifo_queue = malloc(cache.associativity * sizeof(int));
                    for (int j = 0; j < cache.associativity; j++)
                    {
                        cache.sets[i].fifo_queue[j] = -1;
                    }
                    for (int j = 0; j < cache.associativity; j++)
                    {
            ...
            else
            {
                ...
                {
                    cache.sets[0].fifo_queue[j] = -1;
                }
                for (int i = 0; i < numblocks; i++)
                {
                    cache.sets[0].blocks[i].data = malloc(cache.block_size * sizeof(int));
                    cache.sets[0].blocks[i].valid = 0;
                    cache.sets[0].blocks[i].tag = 0;
                    cache.sets[0].blocks[i].dirty = 0;
                    cache.sets[0].blocks[i].lru_counter = 0;
